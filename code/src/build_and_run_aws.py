...
def check_aws_environment() -> None:
  ...
  if aws_profile:
    ...
    logger.info(
      f"AWS_ACCESS_KEY_ID: {'*****' if os.environ.get('AWS_ACCESS_KEY_ID') else 'not set'}")
...
def build_and_push_docker_image(
  ...
  try:
    ...
    validate_docker_args("build", "-t", f"{image_name}:{image_tag}", "-f", dockerfile_path,
                         build_context)
    subprocess.run(
      [docker_exe, "build", "-t", f"{image_name}:{image_tag}", "-f", dockerfile_path,
       build_context],
      check=True,
    )
